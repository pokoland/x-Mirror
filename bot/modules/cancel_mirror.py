...
def cancel_mirror(update, context):
    ...
    if len(args) > 1:
        ...
        if not dl:
            sendMessage(f"GID : <code>{gid}</code> Introuvable.", context.bot, update)
            return
    ...
    if dl.status() == MirrorStatus.STATUS_ARCHIVING:
        sendMessage("Archivage en cours, vous ne pouvez pas l'annuler.", context.bot, update)
    elif dl.status() == MirrorStatus.STATUS_EXTRACTING:
        sendMessage("Extraction en cours, vous ne pouvez pas l'annuler.", context.bot, update)
    elif dl.status() == MirrorStatus.STATUS_SPLITTING:
        sendMessage("Découpage en cours, vous ne pouvez pas l'annuler.", context.bot, update)
    else:
...
def cancel_all(update, context):
    ...
    while True:
        ...
        if dl:
            ...
            break
    sendMessage(f'{count} téléchargement(s) ont été annulés !', context.bot, update)
