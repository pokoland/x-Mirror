...
def clean_download(path: str):
    if os.path.exists(path):
        LOGGER.info(f"Nettoyage du téléchargement : {path}")
        shutil.rmtree(path)
...
def exit_clean_up(signal, frame):
    try:
        LOGGER.info("Veuillez patienter pendant le nettoyage des téléchargements et l'arrêt des téléchargements en cours")
        clean_all()
        sys.exit(0)
    except KeyboardInterrupt:
        LOGGER.warning("Forçage de l'arrêt avant la fin du nettoyage !")
        sys.exit(1)
...
def get_base_name(orig_path: str):
    if orig_path.endswith(".tar.bz2"):
        ...
        raise NotSupportedExtractionArchive('Format de fichier non supporté pour l\'extraction')
